...
def result_to_json(string, tags):
    ...
    for char, tag in zip(string, tags):
        if tag[0] == "S":
            ...
            item["entities"].append({"word": char, "type": tag[2:]})
        elif tag[0] == "B":
            entity_name += char
            entity_start = idx
        elif tag[0] == "I":
            ...
            item["entities"].append({"word": entity_name,"type": tag[2:]})
            entity_name = ""
        else:
            entity_name = ""
            entity_start = idx
    ...
    for c_idx in range(len(tags)):
        ...
        if tag[0] == 'B':
            ...
            if tag_next[2:] != entity_tag:
                ...
                item["entities"].append({"word": c,"type": tag[2:]})
        elif tag[0] == "I":
            if tag[2:] != tags[c_idx-1][2:] or tags[c_idx-1][2:] == 'O':
                ...
                if tag_next[2:] != entity_tag:
                    ...
                    item["entities"].append({"word": entity_name,"type": entity_tag})
                    entity_name = ''
        iCount += 1
    return item
